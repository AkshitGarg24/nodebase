    ...
    <>
      ...
      `}
        ...
        <div className={cn("absolute inset-0 overflow-hidden rounded-[7px]",
          ...
          <div className="spinner rounded-full bg-[conic-gradient(from_0deg_at_50%_50%,_rgba(42,67,233,0.7)_0deg,_rgba(42,138,246,0)_360deg)]" />
        </div>
      </div>
      {children}
    </>
...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    ...
    case "success":
      return (
        <StatusBorder className={cn("border-green-600/70",
          className
        )}>{children}</StatusBorder>
      );
    case "error":
      return <StatusBorder className={cn("border-red-600/70",
        className
      )}>{children}</StatusBorder>;
    default:
      return <>{children}</>;
